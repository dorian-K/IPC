...
bool IPCBridge::initBridge(bool initializeFile) {
    ...
    for (int i = 0; true; i++) {
        this->fileHandle =
            CreateFileW(this->path.c_str(), GENERIC_READ | GENERIC_WRITE,
    ...
    // map to memory
    this->fileMap =
        CreateFileMappingW(this->fileHandle, NULL, PAGE_READWRITE, 0, 0, NULL);
    ...
    this->fileView = reinterpret_cast<decltype(this->fileView)>(MapViewOfFile(
        this->fileMap, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(IPCData) * 2));
...
void IPCBridge::beginMessageLoop() {
    ...
    auto lastOutboundMessage = std::chrono::high_resolution_clock::now() -
    ...
    while (this->isRunningLoop) {
        ...
        if (!this->getOutbound().isListening) {
            ...
            if (std::chrono::duration_cast<std::chrono::milliseconds>(
                    ...
                    disconnectedSince)
                    .count() < 1000)
                this->loopStopNotifier.wait_for(g1,
        ...
        if (hasActiveConnection &&
            std::chrono::duration_cast<std::chrono::seconds>(
                std::chrono::high_resolution_clock::now() - lastInboundMessage)
                    .count() > 3) {
            // Timeout
            dprnt("IPC-Timeout");
            this->getOutbound().isListening = false;
        ...
        if (!this->getOutbound().hasMessage) {
            if (!this->outboundPacketQ.empty()) {
                ...
                memcpy(&this->getOutbound().msg, msg.get(),
                       sizeof(IPCData::IPCMessage));
                this->getOutbound().hasMessage = true;
                lastOutboundMessage = std::chrono::high_resolution_clock::now();
                if (msg->cmd != IPCData::IPCMessage::PING && false)
                    dprnt("Ipcbridge: sent package");
        ...
        if (this->getInbound().hasMessage) {
            if (this->getInbound().msg.cmd != IPCData::IPCMessage::PING &&
                false)
                dprnt("ipcbridge: got package: {}", this->getInbound().msg.cmd);
            hadSomethingToDo = true;
            lastInboundMessage = std::chrono::high_resolution_clock::now();
            // if (this->getInbound().msg.cmd != IPCData::IPCMessage::PING)
            //	dprnt("inbound message! {}", this->getInbound().msg.cmd);
            this->getInbound().msg.dataSize =
                std::min((unsigned int)sizeof(this->getInbound().msg.data),
                         this->getInbound().msg.dataSize);
            ...
            if (msgId != 0) {
                ...
                if (this->callbackMap.count(msgId) > 0) {
                    auto ptr = std::make_shared<IPCData::IPCMessage>(
                        this->getInbound().msg);
                    ...
                    auto e = std::make_shared<IPCData::IPCMessage>(
                        this->getInbound().msg);
        ...
        if (!hadSomethingToDo &&
            boredom > 5) {  // Remove all callbacks which received a timeout
            std::lock_guard g(this->callbackMapMutex);
            auto it = this->callbackMap.begin();
            while (
                (it = std::find_if(
                     it, this->callbackMap.end(),
                     [](std::pair<const unsigned int, CallbackInfo>& entr) {
                         return std::chrono::duration_cast<
                                    std::chrono::seconds>(
                                    std::chrono::high_resolution_clock::now() -
                                    entr.second.startTime)
                                    .count() > entr.second.timeoutSecs;
        ...
        if (hadSomethingToDo)
            ...
            this->loopStopNotifier.wait_for(
                g1, std::chrono::milliseconds((int)boredom + 1));
        }
    }
...
void IPCBridge::enqueuePacket(
    std::shared_ptr<IPCData::IPCMessage> pk,
...
void IPCBridge::enqueuePacket(std::shared_ptr<IPCData::IPCMessage> pk) {
    this->outboundPacketQ.push(pk);
}
