...
class IPCBridge {
   private:
    ...
    bool isClient = false;
    inline IPCData& getInbound() {
        return isClient ? fileView->outgoing : fileView->incoming;
    }
    inline IPCData& getOutbound() {
        return isClient ? fileView->incoming : fileView->outgoing;
    }
    std::optional<std::function<void(std::shared_ptr<IPCData::IPCMessage>&)>>
   ...
   public:
    ...
    void closeBridge();
    void setDefaultPacketHandler(
        std::function<void(std::shared_ptr<IPCData::IPCMessage>&)>
            packetHandler) {
